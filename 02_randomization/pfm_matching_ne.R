...
packages <- c("rgeos", "rgdal", "sp", "raster", "scales", "lubridate", "gdata", "data.table", "dplyr", "Matching", "rgenoud", "stringr", "MatchIt", "tidyverse") 
ipak(packages)
...
#--------------------------------------------------
rm(list=ls())
setwd("X:/Box Sync/08_PanganiFM/PanganiFM/2 - Data and Analysis")
set.seed(1956)
...
df <- read.csv("2 - Final Data/villagescoping.csv")
df.short <- read.csv("2 - Final Data/villagesample_shortlist.csv")
...
#--------------------------------------------------
# Select Only Those Chosen from Scoping
#--------------------------------------------------

df.new <- df.t[df.t$OBJECTID %in% df.short$objectid,]
...
df.treat <- df.new[df.matches$treat,] # Extract treatment matches
df.treat$match <- df.matches$control # Store control match from matches DF
df.control <- df.new[df.matches$control,] # Extract control matches
...
write.csv(df.sample, "2 - Final Data/genmatch_sample_2018.08.01.csv")
